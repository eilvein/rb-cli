    ...
    _.each(commands, function(command) {
        ...
        _.each(command.options || [], function(option) {
            ...
            if (option.defaults !== undefined)
                after.push('Default is ' + option.defaults)
            if (option.values)
                after.push('Values are ' + option.values.join(', '))
...
function exec(commands, command, args, kwargs) {
    let cmd = _.find(commands, function(_cmd) {
    ...
    if (!cmd)
        throw new Error(
            'Command ' +
                command +
                ' doesn\'t exist, run "rb help" to list commands.'
        )

    // Apply defaults
    _.each(cmd.options || [], function(option) {
        kwargs[option.name] =
            kwargs[option.name] === undefined
                ? option.defaults
                : kwargs[option.name]
